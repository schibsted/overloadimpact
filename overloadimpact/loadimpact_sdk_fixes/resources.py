...
class UpdateMixin(object):
    def update(self, data=None):
        ...
        # convert data_stores array from [{id: 123}, {id: 456}] to [123, 456]
        if "data_stores" in data:
            new_data_stores = []
    ...
    def __call__(self, poll_rate=3, post_polls=5):
        ...
        while not done or 0 < post_polls:
            ...
            for rid, data in results.items():
                try:
                    if rid not in self._last: # work around issue with _last key being "__li_requests_per_second:1" instead of "__li_requests_per_second"
                        self._last[rid] = {'offset': -1}
                    if data[-1]['offset'] > self._last[rid]['offset']:
                        change[rid] = data[-1]
