...
def __import_file(file, is_top_file):
    with open(file) as f:
        text = f.read()
        if is_top_file:
            text = __add_header(file, text)
            text = __add_footer(file, text)
        text = __load_imports(file, text)
        return text
...
def __build_code(scenario, versions):
    ...
    file = '%s/%s.lua' % (paths.SCENARIOS_CODE_DIR, scenario)
    text = __import_file(file, True)
