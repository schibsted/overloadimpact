...
def determine_run_data_dir():
    if "OIMP_PROJECT_RUN_DATA_HOME" in os.environ:
        ...
        print('OIMP_PROJECT_RUN_DATA dir not found. Set it as OIMP_PROJECT_RUN_DATA_HOME env var, or execute from'
              'it\'s root dir.\nYou can set up a new project with: oimp setup_project [name] [destination path].')
        return False
    # if we have executed this command from a oimp-suite dir then it will contain /lua/scenarios
    current_path = os.getcwd()
    ...
    print('OIMP_PROJECT_RUN_DATA dir not found. Set it as OIMP_PROJECT_RUN_DATA_HOME env var, or execute from it\'s'
          'root dir.\nYou can set up a new project with: oimp setup_project [name] [destination path].')
    return False
