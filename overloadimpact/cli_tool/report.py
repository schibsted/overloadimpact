...
def pygal_line():
    ...
    chart = pygal.Line(fill=False, style=CleanStyle, show_minor_x_labels=False, x_labels_major_count=10,
                       x_label_rotation=90, width=1000, height=500)
    return chart


def pygal_bar(**kwargs):
    args = {"fill": False, "style": CleanStyle, "show_minor_x_labels": False, "x_labels_major_count": 10,
            "x_label_rotation": 90, "width": 1000, "height": 800, "truncate_label": 40, "truncate_legend": 40,
...
def make_time_chart(report_path, metrics, title, chart_name, run_id, y_label, rows, y_key, dest_dir,
    ...
    if display_active_clients:
        if y_vals[-1] == 1 and y_vals[0] == 1:
            y_vals[-1] -= 0.000001  # work around to avoid render errors when all y_vals are exactly 1
...
def make_time_labels(timestamps):
    time_labels = map(lambda timestamp: datetime.datetime.fromtimestamp(__to_secs(timestamp)).strftime("%H:%M:%S"),
                      timestamps)
    return time_labels
...
def link_title(relative_file_path, title, sub_title):
    return """
<div class="link_title"><h3><a href="%s">%s</a></h3>
<span class="link_subtitle" style="font-style: italic">%s</span></div>
""" % (relative_file_path, title, sub_title)
