...
def __create_page(program_runs):
    ...
    f = open(report_file_path, 'w')  # Trying to create a new file or open one
    f.write(__page_markup(program_runs))
    f.close()
...
def __program_runs_markup(program_runs):
    ...
    for program_run_id in program_runs:
        run_list_markup += report.link_title(
            paths.relative_path(__report_url(program_runs), program_report.complete_report_url(program_run_id)),
            "Program_run " + program_run_id, __program_run_1line_summary(program_runs[program_run_id]))
    ...
    for program_run_name, program_run in program_runs.iteritems():
        ...
        for config_name, config_run in program_run['config_runs'].iteritems():
            ...
            for scenario_name in config_run['config_params']['scenarios']:
                ...
                sets[program_run_name][scenario_name] = config_report.scenario_peak_core_actions_per_sec_avg(
                    scenario_name, config_run['enriched_metrics'], scenario_params)
...
def __target_chart(scenario_name, program_runs):
    ...
    for program_run_name, program_run in program_runs.iteritems():
        ...
        for config_name, config_run in program_run['config_runs'].iteritems():
            ...
            if scenario_name in config_run['config_params']['scenarios']:
                ...
                # if we have an empty entry here from previous iterations, then remove it, since we have found a
                #  valid one
                if program_run_name in entries:
                    entries.pop(program_run_name, None)
                program_run_title = "%s (%d vu's)" % (
                    program_run_name, int(config_params['users'] * (scenario_params["percent-of-users"] / 100.0)))
                entries[program_run_title] = config_report.scenario_peak_core_actions_per_sec_avg(
                    scenario_name,
                    config_run['enriched_metrics'],
                    scenario_params)
    ...
    chart.x_labels = map(lambda key: key.replace(shared_prefix, "", 1) if shared_prefix else key,
                         map(__format_set_title, sorted_keys))
    ...
    #             config_params = config_run['config_params']
    #             scenario_params = config_params['scenarios'][scenario_name]
    #             program_run_title = "%s (%d vu's)" % (program_run_name, int(config_params['users'] * (
    #                 scenario_params["percent-of-users"] / 100.0)))
    #             entries[program_run_title] = config_report.scenario_peak_core_actions_per_sec_avg(
    #                 scenario_name, config_run['enriched_metrics'])
    #         else:
    #             entries[program_run_name] = None
